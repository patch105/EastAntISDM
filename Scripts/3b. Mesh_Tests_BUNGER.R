...
ice_free.EastAnt <- crop(ice_free.EastAnt, ext(bunger_boundary))
...
PA_bunger23_Veg_sf <- st_read(here("Data/Biological_records", "PA_Veg_vestfold.shp"))
...
PA_bunger23 <- PA_bunger23 %>% mutate(area = 80)
PA_bunger23 <- PA_bunger23 %>% mutate(area = 1)

# # Add together
# PA_both <- rbind(PA_bunger23, PA_bunger23)
...
covs <- crop(covs, ext(bunger_boundary))
...
ACBRS_buffer1km <- crop(vect(ACBRS_buffer1km), ext(bunger_boundary))
...
# Crop PO to Vestfold
PO.sf <- crop(vect(PO.sf), ext(bunger_boundary)) %>% 
  st_as_sf()
...
# saveRDS(mesh.range.10km.cutoff.50, file = here("Outputs/Testing/BUNGER_mesh.range.10km.cutoff.50.rds"))
...
# Plot the mesh:
 ...
 ggsave(plot = p1, filename = here("Outputs/Testing/Bungermesh_range_10km_cutoff_50_VESTFOLD.png"), width = 10, height = 10, dpi = 300)
...
m.PA.no.GRF <- isdm(observationList = list(PAdat = PA_bunger23),
                 covars = covs,
                 mesh = mesh.range.10km.cutoff.50,
                 responseNames = c(PA = "presence"),
                 sampleAreaNames = c(PA = "area"),
...
# PA_bunger23$area <- 1
# m.PA.GRF.1km <- isdm(observationList = list(PAdat = PA_bunger23),
#                      covars = covs,
#                      mesh = mesh.range.10km.cutoff.50,
#                      responseNames = c(PA = "presence"),
#                      sampleAreaNames = c(PA = "area"),
...
ggplot() +
        geom_density(data = PA_bunger23_covs, 
                     aes(x = .data[[names(PA_bunger23_covs)[5]]], fill = as.factor(presence)), 
                     alpha = 0.5) +
        theme_bw() +
        labs(title = names(PA_bunger23_covs)[5])
