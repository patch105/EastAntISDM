...
scenario = "Plantarctica_Ensemble_Nov_11"


# Set outpath -------------------------------------------------------------

outpath <- here("Outputs", "Ensemble_Plantarctica", group, scenario)
...
PO_East_Ant_Veg_No_Bunger_sf <- st_read(here("Data/Biological_records", "PO_Plantarctica_No_Bunger.shp"))
PO_East_Ant_Veg_No_Vestfold_sf <- st_read(here("Data/Biological_records", "PO_Plantarctica_No_Vestfold.shp"))
...
PO_East_Ant_Veg_No_Vestfold_df <- PO_East_Ant_Veg_No_Vestfold_sf %>% 
  st_coordinates() %>%
  as.data.frame() %>% 
  bind_cols(st_drop_geometry(PO_East_Ant_Veg_No_Vestfold_sf)) %>% 
  rename(x = X, y = Y)  
...
if(group == "Moss") {
  ...
  PO_No_Vestfold <- PO_East_Ant_Veg_No_Vestfold_df %>% 
    filter(Vegetation == "Vegetation") %>% 
    dplyr::select(x, y) %>% 
    mutate(Presence = 1)
  
  PO_No_Bunger <- PO_East_Ant_Veg_No_Bunger_df %>% 
    filter(Vegetation == "Vegetation") %>% 
    dplyr::select(x, y) %>% 
    mutate(Presence = 1)
  
  PO_No_Vestfold.sf <- PO_East_Ant_Veg_No_Vestfold_sf %>% 
    filter(Vegetation == "Vegetation") %>% 
    mutate(Presence = 1)
  
  PO_No_Bunger.sf <- PO_East_Ant_Veg_No_Bunger_sf %>% 
    filter(Vegetation == "Vegetation") %>% 
    mutate(Presence = 1)
...
PO_datasets <- list("No_Bunger", "No_Vestfold")

map(PO_datasets, function(dataset){
  
  if(dataset == "No_Bunger"){
    
    PO <- PO_No_Bunger
    PO.sf <- PO_No_Bunger.sf
  ...
  if(group == "Lichen") {
    ...
    wt <- ifelse(train_PB_covs$Presence == 1, 1, prNum / bgNum) # down-weighting absences
  ...
  dir.create(file.path(outpath, "Maxent_outputs", dataset), showWarnings = F, recursive = TRUE)
  ...
  writeRaster(pred_cur.mxt.rast, here(outpath, "Maxent_outputs", dataset, paste0("Prediction_East_Antarctica.tif")), overwrite = T)
  ...
  # Make folder:
  dir.create(file.path(outpath, "LASSO_outputs", dataset), showWarnings = F, recursive = TRUE)
  ...
  png(paste0(outpath,  "/LASSO_outputs/", dataset, "/lasso_lambda_plot.png"), width = 800, height = 600)
  ...
  png(paste0(outpath,  "/LASSO_outputs/", dataset, "/lasso_CV_plot.png"), width = 800, height = 600)
  plot(lasso_cv)
  dev.off()
  ...
  png(paste0(outpath,  "/LASSO_outputs/", dataset, "/lasso_deviance_plot.png"), width = 800, height = 600)
  ...
  writeRaster(pred_cur.lasso.rast, here(outpath, "LASSO_outputs", dataset, paste0("Prediction_East_Antarctica.tif")), overwrite = T)
  ...
  dir.create(file.path(outpath, "GAM_outputs", dataset), showWarnings = F, recursive = TRUE)
  ...
  write.csv(gam_kcheck, file = file.path(outpath, "GAM_outputs", dataset, paste0("GAM_kcheck.csv")))
  
  png(paste0(outpath,  "/GAM_outputs/", dataset, "/GAM.check_plot.png"), width = 800, height = 600)
  par(mfrow = c(2, 2))  
  ...
  png(paste0(outpath, "/GAM_outputs/", dataset, "/GAM_response_residual_plot.png"), width = 800, height = 600)
  ...
  write.csv(mod_summary_df, file = file.path(outpath, "GAM_outputs", dataset, paste0("GAM_summary_df.csv")))
  
  png(paste0(outpath,  "/GAM_outputs/", dataset, "/GAM_pdp_plots.png"), width = 800, height = 600)
  ...
  writeRaster(pred_cur.gam.rast, here(outpath, "GAM_outputs", dataset, paste0("Prediction_East_Antarctica.tif")), overwrite = T)
  ...
  dir.create(file.path(outpath, "RF_outputs", dataset), showWarnings = F, recursive =T)
  ...
  # There are more presences than backgrounds in moss so it's the opposite
  ...
  write.csv(oob_error, file = file.path(outpath, "RF_outputs", dataset, paste0("RF_OOB_error.csv")))
  
  confusion_matrix <- rf$confusion
  write.csv(confusion_matrix, file = file.path(outpath, "RF_outputs", dataset, paste0("RF_confusion_matrix.csv")))
  ...
  png(paste0(outpath, "/RF_outputs/", dataset, "/RF_error_vs_trees_plot.png"), width = 800, height = 600)
  plot(rf, main = "RF down-sampled")
  dev.off()
  
  # Plot variable importance
  png(paste0(outpath, "/RF_outputs/", dataset, "/RF_var_importance_Gini_plot.png"), width = 800, height = 600)
  varImpPlot(rf, type = 2) # Gini impurity index
  dev.off()
  
  png(paste0(outpath, "/RF_outputs/", dataset, "/RF_var_importance_Permutation_plot.png"), width = 800, height = 600)
  varImpPlot(rf, type = 1) # Permutation
  dev.off()
  
  # Partial dependence plots
  png(paste0(outpath,  "/RF_outputs/", dataset, "/RF_pdp_plots.png"), width = 800, height = 600)
  flexsdm::p_pdp(rf,
                 predictors = cov_names,
                 resolution = 100,
  ...
  writeRaster(pred_cur.rf.rast, here(outpath, "RF_outputs", dataset, paste0("Prediction_CURRENT.tif")), overwrite = T)
  ...
  dir.create(file.path(outpath, "BRT_outputs", dataset), showWarnings = F, recursive = TRUE)
  ...
  while(is.null(brt)){
    ...
    png(paste0(outpath,  "/BRT_outputs/", dataset, "/BRT_holdout_deviance_cv_plot.png"), width = 800, height = 400)
  ...
  if(is.null(brt)) {
    ...
    write.csv(model_performance, file = file.path(outpath, "BRT_outputs", dataset, paste0("BRT_model_performance.csv")))
    
    # Variable importance
    png(paste0(outpath,  "/BRT_outputs/", dataset, "/BRT_variable_importance_plot.png"), width = 800, height = 800)
    ...
    write.csv(var.imp, file = file.path(outpath, "BRT_outputs", dataset, paste0("BRT_variable_importance.csv")))
    
    # Partial dependence plots & fitted vs. covariate plots
    
    png(paste0(outpath,  "/BRT_outputs/", dataset, "/BRT_partial_effects_plot.png"), width = 800, height = 800)
    ...
    png(paste0(outpath,  "/BRT_outputs/", dataset, "/BRT_fitted_vs_covariate_plot.png"), width = 800, height = 400)
    ...
    write.csv(interactions, file = file.path(outpath, "BRT_outputs", dataset, paste0("BRT_interactions.csv")))
    ...
    writeRaster(pred_cur.brt.rast, here(outpath, "BRT_outputs", dataset, paste0("Prediction_East_Antarctica.tif")), overwrite = T)
  ...
  write.csv(pred_cur_ensemble, file = here(outpath, paste0("Prediction_ensemble_East_Antarctica_", dataset, ".csv")))
  writeRaster(pred_cur_ensemble.rast, here(outpath, paste0("Prediction_ensemble_East_Antarctica_", dataset, ".tif")), overwrite = T)
  ...
  fit <- evaluate_fit_PO_ensemble(x = pred_with_PO,
                                  ...
                                  outpath = file.path(outpath, dataset))
  
  eval_df.ens <- data.frame(model = "Ensemble",
                            validation_dataset = paste0("Training data ", dataset),
                            fit[["fit_df"]])
  
  
  
  if(dataset == "No_Vestfold"){ # If it's the dataset with no Vestfold records
  ...
  if(dataset == "No_Bunger"){ # If it's the dataset with no Bunger records
  ...
  write.csv(eval_df.ens, file = here(outpath, paste0("Ensemble_eval_df_",  dataset, ".csv")))
  ...
  if(dataset == "No_Vestfold"){ # If the dataset with no vestfold, then plot Vestfold
  ...
  if(dataset == "No_Bunger"){ # If the dataset with no bunger, then plot bunger
...
# FINALLY - load the two eval datasets and combine ------------------------

eval_no_bunger <-  read.csv(here(outpath, "Ensemble_eval_df_No_Bunger.csv"))
eval_no_vestfold <-  read.csv(here(outpath, "Ensemble_eval_df_No_Vestfold.csv"))

eval_df <- rbind(eval_no_bunger, eval_no_vestfold) %>% 
  dplyr::select(!X)

write.csv(eval_df, here(outpath, "Ensemble_eval_df.csv"))