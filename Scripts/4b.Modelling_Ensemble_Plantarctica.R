...
PO_East_Ant_Veg_sf <- st_read(here("Data/Biological_records", "PO_Plantarctica.shp"))
...
dir.create(file.path(outpath, "Maxent_outputs"), showWarnings = F, recursive = TRUE)
...
# Make folder:
dir.create(file.path(outpath, "LASSO_outputs"), showWarnings = F, recursive = TRUE)
...
dir.create(file.path(outpath, "GAM_outputs"), showWarnings = F, recursive = TRUE)
...
dir.create(file.path(outpath, "RF_outputs"), showWarnings = F, recursive =T)
...
dir.create(file.path(outpath, "BRT_outputs"), showWarnings = F, recursive = TRUE)
...
dir.create(file.path(outpath), showWarnings = F, recursive = TRUE)
...
eval_df.ens <- data.frame(model = "Ensemble",
                          validation_dataset = paste0("Training data"),
                          fit[["fit_df"]])
...
write.csv(eval_df.ens, file = here(outpath, paste0("Ensemble_eval_df.csv")))
