...
# This code tests the environmental coverage of the Vestfold and Vestfold Hills sites and looks at how different types of data represent the environmental conditions at the sites.
...
##########################################################################
## EXTRACT ENVIRO. CONDITIONS COVERED BY DATASETS AND AT VESTFOLD HILLS ##
##########################################################################
...
PA_Bunger_vect <- st_read(here("Data/Biological_records", "PA_Veg_bunger23.shp"))

PA_Bunger_df <- terra::extract(EastAnt, PA_Bunger_vect, xy = FALSE, na.rm = TRUE) %>% 
  dplyr::select(-ID) %>% 
  mutate(Region = "PA Bunger")
...
##########################################################################
###### PLOTTING ENVIRO. CONDITIONS VESTFOLD HILLS VS. DATA TYPES ############
##########################################################################


# LICHEN ------------------------------------------------------------------

all_lichen <- rbind(vestfold.df, PO_lichen_df, PA_Bunger_df, Plantarctica_lichen_df)

all_lichen$Region <- factor(all_lichen$Region, levels = c("PO Lichen", "Satellite lichen", "PA Bunger", "Vestfold Hills"))

all_lichen$Region <- factor(all_lichen$Region, levels = c("PA Bunger", "Satellite lichen", "PO Lichen", "Vestfold Hills"))
...
# Save combined plot
ggsave(here("Outputs/Figures", "Combined_LICHEN_Distribution_Plots_500m_DENSITY_VESTFOLD.png"),
       plot = combined_plot,
...
# Save combined plot
ggsave(here("Outputs/Figures", "Combined_LICHEN_Distribution_Plots_500m_HISTOGRAM_VESTFOLD.png"),
       plot = combined_plot,
       width = 13, height = 6 + 3 * ceiling(length(plot_list) / 2), dpi = 600)
...
all_moss <- rbind(vestfold.df, PO_moss_df, PA_Bunger_df, Plantarctica_moss_df)

# all_moss$Region <- factor(all_moss$Region, levels = c("PO Moss", "Satellite vegetation", "PA Vestfold", "Vestfold Hills"))

all_moss$Region <- factor(all_moss$Region, levels = c("PA Bunger", "Satellite vegetation", "PO Moss", "Vestfold Hills"))
...
# Save combined plot
ggsave(here("Outputs/Figures", "Combined_MOSS_Distribution_Plots_500m_DENSITY_VESTFOLD.png"),
       plot = combined_plot,
       width = 13, height = 6 + 3 * ceiling(length(plot_list) / 2), dpi = 600)
...
# Save combined plot
ggsave(here("Outputs/Figures", "Combined_MOSS_Distribution_Plots_500m_HISTOGRAM_VESTFOLD.png"),
       plot = combined_plot,
       width = 13, height = 6 + 3 * ceiling(length(plot_list) / 2), dpi = 600)
...
training_PA_Bunger <- PA_Bunger_df %>%
  mutate(Presence = 1) %>% 
  dplyr::select(-Region)%>% 
  dplyr::select(-dist_station)

training_integrated_lichen <- rbind(training_PO_lichen, training_PA_Bunger)
training_integrated_moss <- rbind(training_PO_moss, training_PA_Bunger)
...
training_PA_Plantarctica_lichen <- rbind(training_PA_Bunger, 
                                            training_Plantarctica_lichen)

training_PA_Plantarctica_moss <- rbind(training_PA_Bunger, 
                                          training_Plantarctica_moss)
...
shape_extrap_PA_Bunger <- extra_eval(training_data = training_PA_Bunger,
                                       pr_ab = "Presence",
                                       projection_data = projection,
                                       metric = "mahalanobis",
                                       univar_comb = T,
...
lichen_extrap <- rbind(
  ...
  shape_extrap_PA_Bunger %>% mutate(Region = "PA"),
...
combined_extrap_plot <- annotate_figure(combined_extrap_plot, 
                                        left = textGrob("Environmental distance from Vestfold Hills to training data", 
                                                        rot = 90, vjust = 1, gp = gpar(cex = 1.05)))

# Save the combined plot

ggsave(here("Outputs/Figures", "Combined_Extrapolation_Box_Plots_500m_VESTFOLD.png"),
...
shape_extrap_PA_Bunger_summary <- data.frame(data_type = "PA", 
                                               mean = mean(shape_extrap_PA_Bunger$extrapolation, na.rm = TRUE),
                                               median = median(shape_extrap_PA_Bunger$extrapolation, na.rm = TRUE),
                                               min = min(shape_extrap_PA_Bunger$extrapolation, na.rm = TRUE),
                                               max = max(shape_extrap_PA_Bunger$extrapolation, na.rm = TRUE),
                                               proportion_combinatorial = sum(shape_extrap_PA_Bunger$univar_comb == 2)/ length(shape_extrap_PA_Bunger$univar_comb))
...
extrap_summary <- rbind(shape_extrap_PO_lichen_summary, 
                        ...
                        shape_extrap_PA_Bunger_summary,
...
write.csv(extrap_summary, 
          here("Outputs/Results", "Extrapolation_Summary_500m_VESTFOLD.csv"),
          row.names = FALSE)
...
# Plot the extrapolation across Vestfold Hills
#first removing any rows with NAs in any columns
vestfold.xy.df <- as.data.frame(vestfold, xy = T) %>%
  na.omit()
...
shape_extrap_PO_lichen <- cbind(shape_extrap_PO_lichen, vestfold.xy.df[, c("x", "y")])

shape_extrap_Plantarctica_lichen <- cbind(shape_extrap_Plantarctica_lichen, vestfold.xy.df[, c("x", "y")])

shape_extrap_PA_Bunger <- cbind(shape_extrap_PA_Bunger, vestfold.xy.df[, c("x", "y")])



# MOSS --------------------------------------------------------------------

shape_extrap_PO_moss <- cbind(shape_extrap_PO_moss, vestfold.xy.df[, c("x", "y")])

shape_extrap_Plantarctica_moss <- cbind(shape_extrap_Plantarctica_moss, vestfold.xy.df[, c("x", "y")])

global_min <- min(c(
  shape_extrap_PO_lichen$extrapolation,
  shape_extrap_Plantarctica_lichen$extrapolation,
  shape_extrap_PA_Bunger$extrapolation,
