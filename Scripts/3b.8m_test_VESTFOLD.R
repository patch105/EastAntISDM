...
wind8m <- rast("C:/Users/n11222026/OneDrive - Queensland University of Technology/Data/raw/Vestfold_Hills/Travers_Wind_Vestfold_Layer/REMA_8m_wind_exposure.tif")
...
# Crop PO to Vestfold
PO.sf <- crop(vect(PO.sf), ext(wind8m)) %>% 
  st_as_sf()
...
m.PA.GRF <- isdm(observationList = list(PAdat = PA_vestfold),
                    ...
                    distributionFormula = ~0 + REMA_8m_wind_exposure, # Linear w covs
...
PA_vestfold_covs <- terra::extract(wind8m, PA_vestfold[, c("x", "y")], xy = T)
PA_vestfold_covs <- cbind(PA_vestfold_covs, PA_vestfold["presence"])

ggplot() +
        geom_density(data = PA_vestfold_covs, 
                     aes(x = .data[[names(PA_vestfold_covs)[2]]], fill = as.factor(presence)), 
                     alpha = 0.5) +
        theme_bw() +
        labs(title = names(PA_vestfold_covs)[2])
