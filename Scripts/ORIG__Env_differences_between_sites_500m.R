...
# Stack covariates
EastAnt <- c(TWI, slope, northness, summer_temp, wind_speed)
EastAnt_w_bias <- c(c(TWI, slope, northness, dist_station, summer_temp, wind_speed))
...
for(i in seq_along(cov_names)) {
  ...
  ggsave(filename = here("Outputs/Figures", paste0("Distribution_of_", names(all)[[i]], "_Across_East_Antarctic_Regions_500m.png")),
         plot = plot,
...
for(i in seq_along(names(EastAnt))) {
  ...
  ggsave(filename = here("Outputs/Figures", paste0("PA_MOSS_Vestfold_", names(EastAnt)[i], "_500m.png")),
         plot = plot,
         width = 8, height = 4, dpi = 300, units = "in")
...
for(i in seq_along(names(EastAnt))) {
  ...
  ggsave(filename = here("Outputs/Figures", paste0("PA_LICHEN_Vestfold_", names(EastAnt)[i], "_500m.png")),
         plot = plot,
         width = 8, height = 4, dpi = 300, units = "in")
...
for(i in seq_along(names(EastAnt))) {
  ...
  ggsave(filename = here("Outputs/Figures", paste0("Distribution_of_", names(PO_EASTANT_lichen)[[i]], "_Across_PO_Lichen_500m.png")),
         plot = plot,
         width = 8, height = 4, dpi = 300, units = "in")
...
for(i in seq_along(names(EastAnt))) {
  ...
  ggsave(filename = here("Outputs/Figures", paste0("Distribution_of_", names(PO_EASTANT_moss)[[i]], "_Across_PO_Moss_500m.png")),
         plot = plot,
         width = 8, height = 4, dpi = 300, units = "in")
  
}

# PLOTTING Plantarctica data hints of selection  --------------------------------

Plantarctica_EASTANT_lichen <- rbind(EastAnt.df, Plantarctica_lichen_df)
Plantarctica_EASTANT_lichen$Region <- factor(Plantarctica_EASTANT_lichen$Region, levels = c("Plantarctica Lichen", "East Antarctica"))

for(i in seq_along(names(EastAnt))) {
  
  xmax <- quantile(Plantarctica_EASTANT_lichen[, i], 0.95, na.rm = TRUE)
  
  plot <- ggplot(Plantarctica_EASTANT_lichen, aes(x = .data[[names(Plantarctica_EASTANT_lichen)[[i]]]], y = Region, fill = Region)) +
    geom_density_ridges(scale = 4, alpha = 0.8) +
    scale_y_discrete(expand = c(0, 0)) +     # will generally have to set the `expand` option
    scale_x_continuous(expand = c(0, 0)) +   # for both axes to remove unneeded padding
    coord_cartesian(xlim = c(NA, xmax)) + # to avoid clipping of the very top of the top ridgeline
    scale_fill_manual(values = c(
      "East Antarctica" = "purple4",  
      "Plantarctica Lichen" = "purple"   
    )) +
    theme_classic()+
    labs(x = names(Plantarctica_EASTANT_lichen)[[i]], y = NULL, title = paste0("Distribution of ", names(Plantarctica_EASTANT_lichen)[[i]], " Across East Antarctica & PO Lichen"))
  
  ggsave(filename = here("Outputs/Figures", paste0("Distribution_of_", names(Plantarctica_EASTANT_lichen)[[i]], "_Across_Plantarctica_Lichen_500m.png")),
         plot = plot,
         width = 8, height = 4, dpi = 300, units = "in")
  
}

Plantarctica_EASTANT_moss <- rbind(EastAnt.df, Plantarctica_moss_df)
Plantarctica_EASTANT_moss$Region <- factor(Plantarctica_EASTANT_moss$Region, levels = c("Plantarctica Moss", "East Antarctica"))

for(i in seq_along(names(EastAnt))) {
  
  xmax <- quantile(Plantarctica_EASTANT_moss[, i], 0.95, na.rm = TRUE)
  
  plot <- ggplot(Plantarctica_EASTANT_moss, aes(x = .data[[names(Plantarctica_EASTANT_moss)[[i]]]], y = Region, fill = Region)) +
    geom_density_ridges(scale = 4, alpha = 0.8) +
    scale_y_discrete(expand = c(0, 0)) +     # will generally have to set the `expand` option
    scale_x_continuous(expand = c(0, 0)) +   # for both axes to remove unneeded padding
    coord_cartesian(xlim = c(NA, xmax)) + # to avoid clipping of the very top of the top ridgeline
    scale_fill_manual(values = c(
      "East Antarctica" = "purple4",  # deep orange-red
      "Plantarctica Moss" = "purple"   # warm coral-orange
    )) +
    theme_classic()+
    labs(x = names(Plantarctica_EASTANT_moss)[[i]], y = NULL, title = paste0("Distribution of ", names(Plantarctica_EASTANT_moss)[[i]], " Across East Antarctica & PO Moss"))
  
  ggsave(filename = here("Outputs/Figures", paste0("Distribution_of_", names(Plantarctica_EASTANT_moss)[[i]], "_Across_Plantarctica_Moss_500m.png")),
         plot = plot,
         width = 8, height = 4, dpi = 300, units = "in")
...
for(i in seq_along(cov_names)) {
  ...
  ggsave(filename = here("Outputs/Figures", paste0("Distribution_of_", names(all_lichen)[[i]], "_Across_PO_PA_Lichen_vs_Bunger_500m.png")),
         plot = plot,
         width = 8, height = 4, dpi = 300, units = "in")
...
for(i in seq_along(cov_names)) {
  ...
  ggsave(filename = here("Outputs/Figures", paste0("Distribution_of_", names(all_moss)[[i]], "_Across_PO_PA_Moss_vs_Bunger_500m.png")),
         plot = plot,
         width = 8, height = 4, dpi = 300, units = "in")
...
# Save combined plot
ggsave(here("Outputs/Figures", "Combined_Distribution_Plots_500m.png"),
       plot = combined_plot,
...
shape_extrap_Plantarctica_lichen_summary <- data.frame(data_type = "Plantarctica Lichen",
...
shape_extrap_Plantarctica_moss_summary <- data.frame(data_type = "Plantarctica Moss",
...
shape_extrap_PO_PA_Plantarctica_lichen_summary <- data.frame(data_type = "PO + PA + Plantarctica Lichen",
...
shape_extrap_PO_PA_Plantarctica_moss_summary <- data.frame(data_type = "PO + PA + Plantarctica Moss",
